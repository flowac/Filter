...
(function () {
    ...
    class StorageManager {
         ...
         * Returns object: {status: 'seen_duplicate'|'original_seen'|'new', reason: string, distance: number}
         */
            ...
            const checkScope = (s) => {
                ...
                if (this.db[s][h]) {
                    ...
                    if (currentId && entryData.origin === currentId) {
                        ...
                        return {
                            status: 'seen_duplicate',
                            reason: 'Exact match (same hash)',
                            distance: 0
                        };
                    }
                }
                ...
                if (threshold >= 0 && sim !== null) {
                    for (const key in this.db[s]) {
                        ...
                        if (dist <= threshold) {
                            return {
                                status: 'seen_duplicate',
                                reason: `Fuzzy match (Hamming distance: ${dist}, threshold: ${threshold})`,
                                distance: dist
                            };
                        }
                    }
                ...
                return isKnownSelf ? { status: 'original_seen' } : null;
            };
    ...
    function processContent() {
        ...
        items.forEach(item => {
            ...
            const result = storage.check(text, currentId);

            if (result.status === 'seen_duplicate') {
                hideItem(item, text, result);
            } else if (result.status === 'new') {
                storage.markSeen(text, currentId);
                item.dataset.agProcessed = "true";
            } else if (result.status === 'original_seen') {
                // It's the original, just mark processed so we don't re-check constantly
    ...
    function hideItem(item, text, matchInfo) {
        // Log the hide reason with better preview
        const lines = text.split('\n').filter(l => l.trim());
        const title = lines[0] || text.substring(0, 100);
        const preview = title.substring(0, 120).replace(/\s+/g, ' ');

        console.log(
            `%c[Antigravity] Hidden: %c${matchInfo.reason}`,
        ...
        placeholder.title = `${matchInfo.reason}\n\n${text.substring(0, 200)}...`;
